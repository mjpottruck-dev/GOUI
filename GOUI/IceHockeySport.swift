...
struct IceHockeySport: SportDefinition {
    let id = SportCatalog.iceHockeyID
    let displayName = "Ice Hockey"
    ...
    let periods: [PeriodDefinition] = [
        PeriodDefinition(name: "1st Period", duration: 15 * 60, maxCount: 1),
        PeriodDefinition(name: "2nd Period", duration: 15 * 60, maxCount: 1),
        PeriodDefinition(name: "3rd Period", duration: 15 * 60, maxCount: 1)
    ]
...
private let _iceHockeySportRegistration = SportCatalog.register(IceHockeySport())
